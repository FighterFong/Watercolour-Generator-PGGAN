    ...
    def add_images(self, img):
        ...
        for lod in xrange(len(self.h5_lods)):
            while img.shape[2] > self.resolution / (2 ** lod):
                ...
                img = (img[:, :, 0::2, 0::2] + img[:, :, 0::2, 1::2] + img[:, :, 1::2, 0::2] + img[:, :, 1::2,
                                                                                               1::2]) * 0.25
...
def compare(first_h5, second_h5):
    ...
    if shape_a[1] != shape_b[1]:
        ...
        print(
        'The datasets have different resolution: %dx%d vs. %dx%d' % (shape_a[3], shape_a[2], shape_b[3], shape_b[2]))
    else:
...
def create_celeba_hq(h5_filename, celeba_dir, delta_dir, num_threads=4, num_tasks=100):
    ...
    if len(glob.glob(glob_pattern)) != glob_expected:
        ...
        return
    with open(os.path.join(celeba_dir, 'list_landmarks_celeba.txt'), 'rt') as file:
    ...
    def process_func(idx):
        ...
        kdf = cryptography.hazmat.primitives.kdf.pbkdf2.PBKDF2HMAC(algorithm=algorithm, length=32, salt=orig_file,
